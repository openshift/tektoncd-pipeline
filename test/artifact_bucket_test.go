...
func TestStorageBucketPipelineRun(t *testing.T) {
		...
		Spec: v1beta1.TaskSpec{
			Steps: []v1beta1.Step{{
				Container: corev1.Container{
					Name: "addfile", Image: "public.ecr.aws/ubuntu/ubuntu:latest",
				},
				Script: "echo '#!/bin/bash\necho hello' > /workspace/helloworldgit/newfile",
			}, {
				Container: corev1.Container{
					Name: "make-executable", Image: "public.ecr.aws/ubuntu/ubuntu:latest",
				},
	...
	readFileTask := &v1beta1.Task{
		...
		Spec: v1beta1.TaskSpec{
			Steps: []v1beta1.Step{{Container: corev1.Container{
				Name: "runfile", Image: "public.ecr.aws/ubuntu/ubuntu:latest",
