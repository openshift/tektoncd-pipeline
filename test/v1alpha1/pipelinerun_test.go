...
func TestPipelineRun(t *testing.T) {
	...
	tds := []tests{{
		...
		testSetup: func(ctx context.Context, t *testing.T, c *clients, namespace string, index int) {
			...
			task := tb.Task(getName(taskName, index), tb.TaskSpec(
				tb.Step("public.ecr.aws/ubuntu/ubuntu:latest",
...
func getFanInFanOutTasks() []*v1alpha1.Task {
	...
	return []*v1alpha1.Task{
		tb.Task("create-file", tb.TaskSpec(
			...
			tb.Step("public.ecr.aws/ubuntu/ubuntu:latest", tb.StepName("write-data-task-0-step-0"), tb.StepCommand("/bin/bash"),
				tb.StepArgs("-c", "echo stuff > $(outputs.resources.workspace.path)/stuff"),
			),
			tb.Step("public.ecr.aws/ubuntu/ubuntu:latest", tb.StepName("write-data-task-0-step-1"), tb.StepCommand("/bin/bash"),
		...
		tb.Task("check-create-files-exists", tb.TaskSpec(
			...
			tb.TaskOutputs(outWorkspaceResource),
			tb.Step("public.ecr.aws/ubuntu/ubuntu:latest", tb.StepName("read-from-task-0"), tb.StepCommand("/bin/bash"),
				tb.StepArgs("-c", "[[ stuff == $(cat $(inputs.resources.workspace.path)/stuff) ]]"),
			),
			tb.Step("public.ecr.aws/ubuntu/ubuntu:latest", tb.StepName("write-data-task-1"), tb.StepCommand("/bin/bash"),
		...
		tb.Task("read-files", tb.TaskSpec(
			...
			tb.Step("public.ecr.aws/ubuntu/ubuntu:latest", tb.StepName("read-from-task-1"), tb.StepCommand("/bin/bash"),
...
func getFailingCondition(namespace string) *v1alpha1.Condition {
	return tb.Condition(cond1Name, tb.ConditionNamespace(namespace), tb.ConditionSpec(tb.ConditionSpecCheck("", "public.ecr.aws/ubuntu/ubuntu:latest",
