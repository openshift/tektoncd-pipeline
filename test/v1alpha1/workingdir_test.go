...
func TestWorkingDirCreated(t *testing.T) {
	...
	task := tb.Task(wdTaskName, tb.TaskSpec(
		tb.Step("public.ecr.aws/ubuntu/ubuntu:latest", tb.StepWorkingDir("/workspace/HELLOMOTO"), tb.StepArgs("-c", "echo YES")),
	))
...
func TestWorkingDirIgnoredNonSlashWorkspace(t *testing.T) {
	...
	task := tb.Task(wdTaskName, tb.TaskSpec(
		tb.Step("public.ecr.aws/ubuntu/ubuntu:latest", tb.StepWorkingDir("/HELLOMOTO"), tb.StepArgs("-c", "echo YES")),
	))
	if _, err := c.TaskClient.Create(ctx, task, metav1.CreateOptions{}); err != nil {
		t.Fatalf("Failed to create Task: %s", err)
	}
